...
ReadRegistry (VOID)
{
  ...
  ret = RegOpenKeyEx (HKEY_LOCAL_MACHINE,
		      FIBER_SUBKEY, 0, KEY_EXECUTE, &hKey);
  if (ret != ERROR_SUCCESS)
    {
  ...
  ret = RegQueryValueEx (hKey,
			 FIBER_SUBKEY_EXTNUM, NULL, NULL, szBuf, &dwBufLen);
  if (ret != ERROR_SUCCESS)
    {
  ...
  dwBufLen = sizeof (szBuf);
  ret = RegQueryValueEx (hKey,
			 FIBER_SUBKEY_TRUSTEXT, NULL, NULL, szBuf, &dwBufLen);
  if (ret != ERROR_SUCCESS)
    {
      printf ("Error: cannot query [%s] from registry.\n",
...
int
CheckFileType (LPSTR szInputFile)
{
  int i;
  HANDLE hFile;
  char szBuf[FIBER_IDENTLEN + 1];
  ...
  for (i = 0; i < iEfs; i++)
    {
      ...
      if (hFile == INVALID_HANDLE_VALUE)
	continue;
      if (SetFilePointer (hFile, ef[i].dwOffset, NULL, FILE_BEGIN) !=
	  0xFFFFFFFF)
	{
...
int
SearchExtension (char *ext)
{
  int i;

  for (i = 0; i < iEfs; i++)
    if (strcmpi (ext, ef[i].szExt) == 0)
      break;
...
int
GetExtension (char *lpszInputFile)
{
  char *pt;
  int n;
...
PrintExts (VOID)
{
  ...
  for (i = 0; i < iEfs; i++)
	...
	{
	  BinaryToHexString (ef[i].szIdent, ef[i].dwIdentLength, szBuf,
			     sizeof (szBuf));
	}
  ...
  *pt++ = '\0';
  ...
  if (!strcmp (FIBER_SUBKEY_EXTNUM, szOption))
      ...
      else if (num > 0)
	{
	  opt = (char) num;
	  ret =
	    RegSetValueEx (hKey, FIBER_SUBKEY_EXTNUM, 0, REG_BINARY, &opt, 1);
	}			/* if ( num >= 256 ){ */
    }
...
VOID
ShellExecuteOpenSync (SHELLEXECUTEINFO * lpsei,
		      BOOL fSynchronous, BOOL fDelete)
{
  BOOL fRet;
  DWORD dwRet;
...
PrintUsage (VOID)
{
  ...
  printf
    ("Option:     -e ext                 assume that the extention is \"ext\"\n");
...
main (int argc, char *argv[])
{
  ...
  while (i < argc)
      ...
      else if (!strcmp (argv[i], "-b"))
	{
	  ...
	  if ((i + 1) < argc)
	    ...
	    {
	      printf
		("Error: verb(action) is required with \'-b\' option.\n");
	      fError = TRUE;
	    }
	}
      else if (!strcmp (argv[i], "-d"))
	{
	  ...
	  if ((i + 1) < argc)
	    ...
	    {
	      printf
		("Error: directory name is required with \'-b\' option.\n");
	      fError = TRUE;
	    }
	}
      else
  ...
  /* eliminate "file:" scheme. */
  if ((strlen (szInputFile) >= 5) && strncmp ("file:", szInputFile, 5) == 0)
    {
