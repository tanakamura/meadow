...
:checkutils
rem In case of cl and nmake, cp and rm are not required.
rem So this test is skipped.
if (%COMPILER%)==(cl) goto checkcompiler
...
echo main(){} >junk.c
gcc -c -O2 -mtune=i686 junk.c
if not errorlevel 1 goto gccMtuneOk
set mf=-mcpu=i686
rm -f junk.c junk.o
goto genmakefiles
:gccMtuneOk
echo GCC supports -mtune=i686
set mf=-mtune=i686
rm -f junk.c junk.o
goto genmakefiles
...
copy /b config.settings+%MAKECMD%.mw32.defs+..\nt\makefile.mw32-in ..\nt\makefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\lib-src\makefile.mw32-in ..\lib-src\makefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\src\makefile.mw32-in ..\src\makefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\man\makefile.w32-in ..\man\makefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\lispref\makefile.w32-in ..\lispref\makefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\lispintro\makefile.w32-in ..\lispintro\makefile
if exist ..\lisp\makefile rm -f ../lisp/[Mm]akefile
copy /b config.settings+%MAKECMD%.mw32.defs+..\lisp\makefile.mw32-in ..\lisp\makefile
rem   Use the default (no-op) Makefile.in if the nt version is not present.
if exist ..\leim\makefile.mw32-in copy /b config.settings+%MAKECMD%.mw32.defs+..\leim\makefile.mw32-in ..\leim\makefile
if not exist ..\leim\makefile.mw32-in copy /b config.settings+%MAKECMD%.mw32.defs+..\leim\Makefile.in ..\leim\makefile

del config.settings

rem
rem check for capabilities of the platform
rem 
...
rem for gcc
set JUNK_SRC=junk.c
set CC_OPT=-DWINVER=0x0400
set JUNK_OBJ=junk.o

if %COMPILER%==gcc goto check_capability
rem for VC
set JUNK_OBJ=junk.obj
set CC_OPT=-nologo %CC_OPT%
...
set TARGET=HAVE_RECONVERTSTRING
...
%COMPILER% %CC_OPT% -c %JUNK_SRC% >config.capability 2>&1
if exist %JUNK_OBJ% goto RECONVERTSTRING_yes
echo ...no
echo #undef %TARGET% >>..\src\config.h
goto RECONVERTSTRING_cnt

:RECONVERTSTRING_yes
echo ...yes
echo #define %TARGET% >>..\src\config.h

:RECONVERTSTRING_cnt
rm -f %JUNK_SRC% %JUNK_OBJ%

echo.>>..\src\config.h
echo /* End of settings from configure.bat.  */>>..\src\config.h

rm config.capability

rem
rem end of checking for capabilities
rem 
