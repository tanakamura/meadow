...
registry_to_reg_sz_size (Lisp_Object data)
{
  ...
  MW32_ENCODE_TEXT (data, Vlocale_coding_system, &text, &size);
  return size + 1;
}
...
registry_to_reg_sz (LPBYTE pdata, DWORD size, Lisp_Object data)
{
  ...
  MW32_ENCODE_TEXT (data, Vlocale_coding_system, &text, &l);
...
registry_get (registry_key *regkey)
{
  ...
  if (regkey->subkey)
    {
      if (RegOpenKeyEx (regkey->hkey, regkey->subkey, 0,
			KEY_READ, &hrkey) != ERROR_SUCCESS)
	{
  ...
  if (err != ERROR_SUCCESS)
    {
      Fsignal (Qerror, Fcons (build_string ("Invalid registry name"),
			      Fcons (build_string (regkey->rootkey),
				     Fcons (build_string (regkey->subkey),
					    build_string (regkey->name)))));
      return Qnil;
    }
...
registry_set (registry_key *regkey, Lisp_Object data)
{
  ...
  if (RegSetValueEx (hrkey, regkey->name, 0,
		     regkey->type, pdata, size) == ERROR_SUCCESS)
    result = Qt;
  else
    result = Qnil;
...
DEFUN ("mw32-registry-list-keys", Fmw32_registry_list_keys,
       ...
       doc: /* Make string list of sub key names under specified KEY.
       ...
       Lisp_Object key, with_data;
{
  ...
  MW32_ENCODE_TEXT (key, Vlocale_coding_system, &key_data, &key_len);
...
DEFUN ("mw32-registry-list-values", Fmw32_registry_list_values,
       ...
       doc: /* Make string list of value names under specified KEY.
If optional argument WITH-DATA is non-nil, returns list of cons of
name string and value. The format of value is same to
`mw32-registry-get'.  */)
       (key, with_data)
...
DEFUN ("mw32-registry-get", Fmw32_rigistry_get, Smw32_registry_get, 1, 2, 0,
       doc: /* Get registry data from KEY or NAME in KEY if specified.
     ...
     Lisp_Object key, name;
{
  ...
  if (STRINGP (name))
    {
      MW32_ENCODE_TEXT (name, Vlocale_coding_system,
			&name_data, &name_len);
    }
  else
    name_data = NULL;
...
DEFUN ("mw32-registry-words-to-integer",
       ...
       2, 4, 0,
       doc: /* Make emacs integer from big integer specified as some 16bit words.
...
DEFUN ("mw32-registry-delete-key", Fmw32_registry_delete_key,
       Smw32_registry_delete_key, 1, 1, 0,
       doc: /*   */
)
     (key)
     Lisp_Object key;
...
DEFUN ("mw32-registry-delete-value", Fmw32_registry_delete_value,
       Smw32_registry_delete_value, 2, 2, 0,
       doc: /*   */
)
     (key, name)
     Lisp_Object key, name;
{
  ...
  MW32_ENCODE_TEXT (key, Vlocale_coding_system, &key_data, &key_len);
  MW32_ENCODE_TEXT (name, Vlocale_coding_system, &name_data, &name_len);
...
DEFUN ("mw32-registry-create-key", Fmw32_registry_create_key,
       Smw32_registry_create_key, 1, 1, 0,
       doc: /*   */
)
     (key)
     Lisp_Object key;
